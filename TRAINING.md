...
Copy `cube_train.py`, `cube_smoke.py`, and `data/cube_shallow_256.jsonl` to the server, preserving the data directory. Use the same Python 3.11 environment that passed the GPU smoke check: torch 2.6.0 CUDA 12.4 and transformers 5.3.0. No additional training dependencies are required.
...
Optional CPU regression check (copy `check_cube_train.py` too):
...
Copy `cube_eval.py` beside `cube_smoke.py`. It uses pure Python facelet permutations verified against cubejs, so the training server does not need Node.js. Run:

```bash
CUDA_VISIBLE_DEVICES=1 python cube_eval.py \
...
```bash
CUDA_VISIBLE_DEVICES=1 python cube_train.py \
  --model "$CUBE_MODEL" \
...
Copy `cube_grpo.py` and `check_cube_grpo.py` alongside the existing `cube_smoke.py`, `cube_train.py`, and `cube_eval.py`. No new package is required. The current implementation starts from an SFT checkpoint; it does not resume RL optimizer/confidence state.

```bash
python check_cube_grpo.py

CUDA_VISIBLE_DEVICES=1 python cube_grpo.py \
  --checkpoint "$HOME/cube-runs/overfit-1000/checkpoint" \
...
Copy updated `cube_grpo.py`, `check_cube_grpo.py`, and new `cube_probe.py` to the server. The original trainer still uses temperature 1; the probe's configurable temperature is for inference only. The collector stores the actual temperature-scaled sampling log-probability. Do not start training at a different temperature without also changing the objective/reference probabilities consistently.
